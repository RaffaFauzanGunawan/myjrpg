  ...
  render(ctx, state, cameraX, cameraY) {
    ...
    for (let y = startTileY; y < endTileY; y++) {
      for (let x = startTileX; x < endTileX; x++) {
        ...
        // Draw pixel-art tile sprite
        const spriteKey = TILE_SPRITE_MAP[tile] || 'grass';
        const tileSprite = getTileSprite(spriteKey);

        // Water animation (two frames)
        if (spriteKey === 'water') {
          const frame = (Math.floor(this.animTime * 4 + x + y) % 2 === 0) ? tileSprite.a : tileSprite.b;
          drawSprite(ctx, frame, screenX, screenY, TILE_SIZE, TILE_SIZE);
        } else {
          drawSprite(ctx, tileSprite, screenX, screenY, TILE_SIZE, TILE_SIZE);
        }

        // Glow overlays for special tiles
        if (tile === T.WAYSTONE) {
          const glow = Math.sin(this.animTime * 4) * 30 + 20;
          ctx.fillStyle = 'rgba(58,90,154,' + (glow / 255) + ')';
          ctx.fillRect(screenX, screenY, TILE_SIZE, TILE_SIZE);
        } else if (tile === T.SAVE) {
          const glow = Math.sin(this.animTime * 3 + 1) * 20 + 10;
          ctx.fillStyle = 'rgba(42,106,74,' + (glow / 255) + ')';
          ctx.fillRect(screenX, screenY, TILE_SIZE, TILE_SIZE);
        } else if (tile === T.BOSS) {
          const pulse = Math.sin(this.animTime * 3) * 0.2 + 0.2;
          ctx.fillStyle = 'rgba(138,26,26,' + pulse + ')';
          ctx.fillRect(screenX, screenY, TILE_SIZE, TILE_SIZE);
        }
      }
    }
    ...
    for (const npc of (area.npcs || [])) {
      ...
      if (screenX > -TILE_SIZE && screenX < VIEWPORT_W + TILE_SIZE &&
        ...
        // NPC pixel-art sprite
        drawSprite(ctx, getNpcSprite(npc.id), screenX + 2, screenY, TILE_SIZE - 4, TILE_SIZE - 2);
    ...
    // Player pixel-art sprite (party leader)
    const leaderId = state.activePartyMembers && state.activePartyMembers[0] ? state.activePartyMembers[0] : 'cedric';
    drawSprite(ctx, getCharacterSprite(leaderId),
      playerScreenX + TILE_SIZE / 2 - 18, playerScreenY + TILE_SIZE / 2 - 22 + bobY, 36, 45);
...
class BattleSystem {
  ...
  startBattle(state, enemyGroup) {
    ...
    // Build turn order immediately so combat starts properly
    this._party = state.party;
    this._activePartyMembers = state.activePartyMembers;
  ...
  calculateTurnOrder(activePartyMembers, party) {
    const combatants = [];
    if (!activePartyMembers || !party) return;
  ...
  update(party, activePartyMembers) {
    ...
    // Auto-drive enemy turns
    if (this.phase === 'enemy_turn') {
      if (this.enemyDelay === undefined) this.enemyDelay = 40;
      this.enemyDelay--;
      if (this.enemyDelay <= 0) {
        this.enemyDelay = undefined;
        this.executeEnemyTurn(this._party, this._activePartyMembers);
      }
    }

    // If battle starts on an enemy's turn, enter enemy_turn phase
    if (this.phase === 'player_select') {
      const cc = this.getCurrentCombatant();
      if (cc && cc.type === 'enemy') {
  ...
  renderEnemies(ctx) {
    ...
    for (let i = 0; i < this.enemies.length; i++) {
      ...
      const spr = getEnemySprite(enemy.id);
      const sprW = spr.width;
      const sprH = spr.height;
      const targetH = enemy.isBoss ? 84 : 56;
      const targetW = sprW * (targetH / sprH);
      drawSprite(ctx, spr, x - targetW / 2, y - targetH / 2, targetW, targetH);
  ...
  renderParty(ctx, state) {
    ...
    for (let i = 0; i < this._activePartyMembers.length; i++) {
      ...
      drawSprite(ctx, getCharacterSprite(id), x - 22, y - 30, 44, 55);
...
class Game {
  ...
  renderTitleScreen() {
    ...
    // Pixel crown
    const crownY = 130 + Math.sin(Date.now() * 0.002) * 8;
    const crownSprite = getCrownSprite();
    drawSprite(ctx, crownSprite, W / 2 - 52, crownY, 104, 78);
  ...
  renderHUD(ctx) {
    ...
    for (const id of this.state.activePartyMembers) {
      ...
      // Portrait sprite
      drawSprite(ctx, getCharacterSprite(id), x + 2, 10, 24, 30);
  ...
  renderMenuParty(ctx) {
    ...
    for (let i = 0; i < allChars.length; i++) {
      ...
      // Portrait sprite
      drawSprite(ctx, getCharacterSprite(id), 240 - 28, cy + 32 - 35, 56, 70);
